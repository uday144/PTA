...
public class Conversation extends AbstractEntity implements Blockable {
    ...
    //lets use this name field to store group name
    private String name;
    ...
    public Conversation(final String name, final Account account, final Jid contactJid,
                        final int mode, final int grpId, int studId) {
        this(java.util.UUID.randomUUID().toString(), name, null, account.getUuid(), contactJid,
                System.currentTimeMillis(), STATUS_AVAILABLE, mode, "", grpId, studId);
        this.account = account;
    }

    public Conversation(final String uuid, final String name, final String contactUuid,
                        final String accountUuid, final Jid contactJid, final long created,
                        final int status, final int mode, final String attributes,
                        final int groupId, final int studentId) {
        this.uuid = uuid;
        this.name = name;
    ...
    public Message getLatestMessage() {
        if (this.messages.size() == 0) {
            Message message = new Message(this, "", Message.ENCRYPTION_NONE, null, -1);

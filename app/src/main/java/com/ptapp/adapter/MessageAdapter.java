...
public class MessageAdapter extends ArrayAdapter<Message> {
    ...
    private View.OnLongClickListener openContextMenu = new View.OnLongClickListener() {
    ...
    private void displayStatus(ViewHolder viewHolder, Message message) {
        ...
        if (viewHolder != null) {
            if (viewHolder.indicatorDisplayed != null) {
                viewHolder.indicatorDisplayed.setVisibility(View.GONE);
            }
            if (viewHolder.indicatorReceived != null) {
                viewHolder.indicatorReceived.setVisibility(View.GONE);
            }
            if (viewHolder.indicatorSent != null) {
                viewHolder.indicatorSent.setVisibility(View.GONE);
            }
            if (viewHolder.indicatorWait != null) {
                viewHolder.indicatorWait.setVisibility(View.GONE);
            }
        }
        ...
        switch (message.getMergedStatus()) {
            case Message.STATUS_WAITING:
                ...
                viewHolder.indicatorWait.setVisibility(View.VISIBLE);
                break;
            ...
            case Message.STATUS_SEND_DISPLAYED:
                viewHolder.indicatorDisplayed.setVisibility(View.VISIBLE);
                break;
            case Message.STATUS_SEND:
                viewHolder.indicatorSent.setVisibility(View.VISIBLE);
                break;
    ...
    private void displayTextMessage(final ViewHolder viewHolder, final Message message, final String senderName) {
        if (viewHolder.download_button != null) {
            viewHolder.download_button.setVisibility(View.GONE);
        }
        viewHolder.image.setVisibility(View.GONE);
        ...
        if (viewHolder.sender_display_name != null) {
            viewHolder.sender_display_name.setText(senderName);
        }
    }
        ...
        if (view == null) {
            ...
            switch (type) {
                ...
                case SENT:
                    ...
                    viewHolder.indicatorDisplayed = (ImageView) view
                            .findViewById(R.id.indicator_displayed);
                    viewHolder.indicatorReceived = (ImageView) view
                            .findViewById(R.id.indicator_received);
                    viewHolder.indicatorSent = (ImageView) view
                            .findViewById(R.id.indicator_sent);
                    viewHolder.indicatorWait = (ImageView) view
                            .findViewById(R.id.indicator_wait);
                    viewHolder.indicatorFlagF = (TextView) view.findViewById(R.id.indicator_flag_f);
                    viewHolder.indicatorFlagM = (TextView) view.findViewById(R.id.indicator_flag_m);
                    break;
                case RECEIVED:
                    view = activity.getLayoutInflater().inflate(
                    ...
                    viewHolder.indicatorReceived = (ImageView) view
                            .findViewById(R.id.indicator_received);
                    viewHolder.sender_display_name = (TextView) view.findViewById(R.id.sender_display_name);
                    break;
                case STATUS:
...
//                viewHolder.contact_picture.setImageBitmap(activity.avatarService().get(conversation.getContact(),
...
//                viewHolder.contact_picture.setImageBitmap(activity.avatarService().get(contact, activity.getPixel(48)));
            ...
            //TODO:Communication is only between parent & teacher
            if (SharedPrefUtil.getPrefUserInRole(activity).equals(CommonConstants.ROLE_PARENT)) {
                senderDisplayName = DatabaseBackend.getStaffName(activity,
                        message.getCounterpartJIds().get(0).toBareJid().toString());

            } else if (SharedPrefUtil.getPrefUserInRole(activity).equals(CommonConstants.ROLE_STAFF)) {
                Cursor c = DatabaseBackend.getParentKidInfo(activity,
                        message.getCounterpartJIds().get(0).toBareJid().toString(),
                        message.getStudentId());
                if (c != null) {
                    senderDisplayName = c.getString(ParentQuery.PAR_FIRST_NAME)
                            + " ( " + c.getString(ParentQuery.STU_FIRST_NAME) + "'s "
                            + c.getString(ParentQuery.PARENT_TYPE) + " )";
                }
            }
            //TODO:getting mom or dad name alongwith student's name from the Jid
        } else if (type == SENT) {
            //SCH: no need
//            viewHolder.contact_picture.setImageBitmap(activity.avatarService().get(account, activity.getPixel(48)));
            /*//TODO:set the visibility of the flags acc. to the msg flags, for teacher side
            for (String s : message.getFlags()) {
                if(s.equals(Message.PARENT_TYPE_F)){
                 viewHolder.indicatorFlagF.setVisibility(View.VISIBLE);
                }
                if(s.equals(Message.PARENT_TYPE_M)){
                    viewHolder.indicatorFlagM.setVisibility(View.VISIBLE);
                }
            }*/
        ...
        if (downloadable != null && downloadable.getStatus() != Downloadable.STATUS_UPLOADING) {
            ...
            displayTextMessage(viewHolder, message, senderDisplayName);
        }
    ...
    private static class ViewHolder {
        ...
        protected ImageView indicatorDisplayed;
        protected ImageView indicatorReceived;
        protected ImageView indicatorSent;
        protected ImageView indicatorWait;
        protected TextView time;
        protected TextView messageBody;
        protected ImageView contact_picture;
        protected TextView status_message;
        protected TextView sender_display_name;
        protected TextView indicatorFlagF;
        protected TextView indicatorFlagM;
    }

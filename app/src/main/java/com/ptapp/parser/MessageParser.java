...
public class MessageParser extends AbstractParser implements OnMessagePacketReceived {
    ...
    //TODO:get message's groupJid and put it in a variable in the conversation view
    private Message parseChat(MessagePacket packet, Account account) {
        Conversation conversation;


        //If it's a message in a group context and not a real peer to peer exchange,
        // then set the from jid to a group jid
        int studentId = -1;


//        if (packet.hasChild("groupjid")) {
//            try {
//                final Jid groupJid = Jid.fromString(packet.getGroupJid());
//                if (packet.hasChild("stuId")) {
//                    studentId = packet.getStudentId();
//                }
//                final int stuId = studentId;
//                conversation = mXmppConnectionService.findOrCreateConversation(account,
//                        groupJid.toBareJid(), false, null, -1, stuId, 0);
...
//        }
        if (packet.hasChild(CommonConstants.M_CUSTOM)) {
            try {
                ...
                //final int groupId = packet.getGroupId();
                //set groupName

//                final int stuId = studentId;
                Cursor c = DatabaseBackend.getGroupRecord(mXmppConnectionService, groupJid.toBareJid().toString());
                if (c != null) {
                    conversation = mXmppConnectionService.findOrCreateConversation(account,
                            groupJid.toBareJid(), false, null,
                            c.getString(c.getColumnIndex(PTAppContract.DefaultGroups.GROUP_NAME)),
                            c.getInt(c.getColumnIndex(PTAppContract.DefaultGroups._ID)), studentId, 0);
                } else {
                    return null;
                }
            } catch (InvalidJidException e) {
            ...
            conversation = mXmppConnectionService.findOrCreateConversation(account, jid.toBareJid(), false, null, 0);
        }
        ...
        /*if (pgpBody != null) {
            finishedMessage = new Message(conversation,
                    pgpBody, Message.ENCRYPTION_PGP, Message.STATUS_RECEIVED, null);
        } else {*/


        ArrayList<Jid> counterpartJids = new ArrayList<>();
        //expecting only one Jid
        counterpartJids.add(packet.getFrom());
        //SCH:
        finishedMessage = new Message(conversation, packet.getBody(), Message.ENCRYPTION_NONE,
                Message.STATUS_RECEIVED, -1, counterpartJids, studentId);
        /*}*/
        finishedMessage.setRemoteMsgId(packet.getId());
        finishedMessage.markable = isMarkable(packet);
        /*if (conversation.getMode() == Conversation.MODE_MULTI && !jid.isBareJid()) {
    ...
    private Message parseOtrChat(MessagePacket packet, Account account) {
        ...
        Conversation conversation = mXmppConnectionService.findOrCreateConversation(account, from.toBareJid(), false, null, 0);
        String presence;
        if (from.isBareJid()) {
            presence = "";
        } else {
    ...
    private Message parseGroupchat(MessagePacket packet, Account account) {
        ...
        Conversation conversation = mXmppConnectionService
                .findOrCreateConversation(account, from.toBareJid(), true, null, 0);
        ...
        if (pgpBody == null) {
            finishedMessage = new Message(conversation,
                    packet.getBody(), Message.ENCRYPTION_NONE, status,null);
        } else {
            finishedMessage = new Message(conversation, pgpBody,
                    Message.ENCRYPTION_PGP, status,null);
        }
        finishedMessage.setRemoteMsgId(packet.getId());
        finishedMessage.markable = isMarkable(packet);
        finishedMessage.setCounterpart(from);
    ...
    private Message parseCarbonMessage(final MessagePacket packet, final Account account) {
        ...
        Conversation conversation = mXmppConnectionService
                .findOrCreateConversation(account, fullJid.toBareJid(), false, null, 0);
        ...
        if (pgpBody != null) {
            ...
            finishedMessage = new Message(conversation, body,
                    Message.ENCRYPTION_NONE, status,null);
        }
    ...
    private Message parseMamMessage(MessagePacket packet, final Account account) {
        ...
        if (from != null && to != null && from.toBareJid().equals(account.getJid().toBareJid())) {
            status = Message.STATUS_SEND;
            conversation = this.mXmppConnectionService.findOrCreateConversation(account, to.toBareJid(), false, query, null, 0);
            counterpart = to;
        } else if (from != null && to != null) {
            status = Message.STATUS_RECEIVED;
            conversation = this.mXmppConnectionService.findOrCreateConversation(account, from.toBareJid(), false, query, null, 0);
            counterpart = from;
        } else {
            return null;
        }
        Message finishedMessage = new Message(conversation, content, encryption, status,null);
    ...
    private void parseNonMessage(Element packet, Account account) {
        ...
        if (invite != null) {
            Conversation conversation = mXmppConnectionService.findOrCreateConversation(account, from, true, null, 0);
        ...
        if (packet.hasChild("event", "http://jabber.org/protocol/pubsub#event")) {
            ...
            mXmppConnectionService.markMessage(account, id, packet, Message.STATUS_SEND_DISPLAYED);
            ...
            mXmppConnectionService.markMessage(account, id, packet, Message.STATUS_SEND_RECEIVED);
            ...
            String id = packet.findChild("received", "urn:xmpp:receipts")
                    .getAttribute("id");
            updateLastseen(packet, account, false);
            //Note : "urn:xmpp:chat-markers:0" or "urn:xmpp:receipts" appears to have common purpose and let's handle them equally.
            mXmppConnectionService.markMessage(account, id, packet, Message.STATUS_SEND_RECEIVED);

            //[Birender] Added Below else block for STATUS_SEND_DISPLAYED
        } else if (from != null
                && packet.hasChild("read", "urn:xmpp:readreceipts")) {
            String id = packet.findChild("read", "urn:xmpp:readreceipts")
                    .getAttribute("id");
            updateLastseen(packet, account, true);
            //Note : "urn:xmpp:chat-markers:0" or "urn:xmpp:receipts" appears to have common purpose and let's handle them equally.
            mXmppConnectionService.markMessage(account, id, packet, Message.STATUS_SEND_DISPLAYED);
    ...
    public void onMessagePacketReceived(Account account, MessagePacket packet) {
        ...
        if ((mXmppConnectionService.confirmMessages()) && ((packet.getId() != null))) {

...
import com.ptapp.activity.TimePreference;
...
import static com.ptapp.app.R.drawable.schoolo;
...
public class NotificationService {
    ...
    private NotificationCompat.Builder buildSingleConversations(final boolean notify) {
        final NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(mXmppConnectionService);
        final ArrayList<Message> messages = notifications.values().iterator().next();
        if (messages.size() >= 1) {
            final Message msg = messages.get(0);
            final Conversation conversation = msg.getConversation();
            //SCH
            /*mBuilder.setLargeIcon(mXmppConnectionService.getAvatarService().get(conversation, getPixel(64)));*/
            Bitmap largeIcon = BitmapFactory.decodeResource(mXmppConnectionService.getResources(), R.drawable.schoolo);
    ...
    private void modifyForImage(final NotificationCompat.Builder builder, final Message message,
    ...
    private void modifyForTextOnly(final NotificationCompat.Builder builder,
                                   final ArrayList<Message> messages, final boolean notify) {
    ...
    //SCH - was not opening the activity, on click of the notification
    ...
    private PendingIntent createContentIntent(final String conversationUuid, final String downloadMessageUuid) {
        ...
        PendingIntent pIntent = PendingIntent.getActivity(mXmppConnectionService, 0, viewConversationIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        return pIntent;
    }

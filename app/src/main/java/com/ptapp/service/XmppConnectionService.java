...
import com.ptapp.activity.UiCallback;
...
import com.ptapp.generator.MessageGenerator;
import com.ptapp.parser.IqParser;
import com.ptapp.parser.MessageParser;
import com.ptapp.parser.PresenceParser;
import com.ptapp.persistence.DatabaseBackend;
import com.ptapp.persistence.FileBackend;
import com.ptapp.entities.Account;
import com.ptapp.entities.Blockable;
import com.ptapp.entities.Bookmark;
...
import com.ptapp.generator.IqGenerator;
import com.ptapp.generator.PresenceGenerator;
import com.ptapp.http.HttpConnectionManager;
...
import com.ptapp.utils.OnPhoneContactsLoadedListener;
import com.ptapp.utils.PRNGFixes;
import com.ptapp.utils.PhoneHelper;
import com.ptapp.utils.Xmlns;
import com.ptapp.xml.Element;
import com.ptapp.xmpp.OnBindListener;
import com.ptapp.xmpp.OnContactStatusChanged;
import com.ptapp.xmpp.OnIqPacketReceived;
import com.ptapp.xmpp.OnMessageAcknowledged;
import com.ptapp.xmpp.OnMessagePacketReceived;
import com.ptapp.xmpp.OnPresencePacketReceived;
import com.ptapp.xmpp.OnStatusChanged;
import com.ptapp.xmpp.OnUpdateBlocklist;
import com.ptapp.xmpp.XmppConnection;
import com.ptapp.xmpp.forms.Data;
import com.ptapp.xmpp.forms.Field;
import com.ptapp.xmpp.jid.InvalidJidException;
import com.ptapp.xmpp.jid.Jid;
import com.ptapp.xmpp.jingle.JingleConnectionManager;
import com.ptapp.xmpp.jingle.OnJinglePacketReceived;
...
//TODO: See options to reduce delay in connection setup with backend (connecting -> online state)
public class XmppConnectionService extends Service implements OnPhoneContactsLoadedListener {
    ...
    //SCH:
    protected static XmppConnectionService xmppConnectionService;

    //SCH: NOTE: setters will be used by XMPPActivity class just once during init. All other classes should invoke Getter only, that too with null check.
    public static XmppConnectionService getXmppConnectionService() {
        return xmppConnectionService;
    }

    //SCH: NOTE: setters will be used by XMPPActivity class just once. All other classes should invoke Getter only, that too with null check.
    public static void setXmppConnectionService(XmppConnectionService xmppConnectionService) {
        XmppConnectionService.xmppConnectionService = xmppConnectionService;
    }
    ...
    private final OnBindListener mOnBindListener = new OnBindListener() {
        ...
        public void onBind(final Account account) {
            ...
            //TODO: Currently we don't need roster, it's an overhead here. Analyze if we really need it in future...
...
//            connectMultiModeConversations(account);
    ...
    public void attachFileToConversation(final Conversation conversation,
        ...
        if (conversation.getNextEncryption(forceEncryption()) == Message.ENCRYPTION_PGP) {
            message = new Message(conversation, "", Message.ENCRYPTION_DECRYPTED, null, -1);
        } else {
            message = new Message(conversation, "", conversation.getNextEncryption(forceEncryption()), null, -1);
        }
    ...
    public void attachImageToConversation(final Conversation conversation,
        ...
        if (conversation.getNextEncryption(forceEncryption()) == Message.ENCRYPTION_PGP) {
            ...
            message = new Message(conversation, "",
                    conversation.getNextEncryption(forceEncryption()), null, -1);
        }
        /*message.setCounterpart(conversation.getNextCounterpart());*/
    ...
    public Conversation find(Bookmark bookmark) {
        return find(bookmark.getAccount(), bookmark.getJid(), -1);
    }

    public Conversation find(final Account account, final Jid jid, final int studentId) {
        return find(getConversations(), account, jid, studentId);
    }
    ...
    public XmppConnection createConnection(final Account account) {
        ...
        account.setResource(sharedPref.getString("resource", "schoolo")
    ...
    public void sendMessage(final Message message) {
        ...
        ArrayList<MessagePacket> packets = null;
        boolean saveInDb = true;
        boolean send = false;
        if (account.getStatus() == Account.State.ONLINE && account.getXmppConnection() != null) {
            if (message.getType() == Message.TYPE_IMAGE || message.getType() == Message.TYPE_FILE) {
                /*if (message.getCounterpart() != null) {*/
                if (!message.getCounterpartJIds().isEmpty()) {
                ...
                if (message.getEncryption() == Message.ENCRYPTION_OTR) {
                    ...
                    packets = mMessageGenerator.generateChat(message);
                    send = true;
                }
            }
        ...
        if ((send) && (null != packets) && (!packets.isEmpty())) {
...
//        MessagePacket packet = null;
        ...
        if (message.getEncryption() == Message.ENCRYPTION_OTR) {
            ...
            if (message.getEncryption() == Message.ENCRYPTION_NONE) {
                packets = mMessageGenerator.generateChat(message, true);
            }
//            else if ((message.getEncryption() == Message.ENCRYPTION_DECRYPTED)
...
//        }
                    ...
                    for (final Element item : storage.getChildren()) {
                        if (item.getName().equals("conference")) {
                            ...
                            if (conversation != null) {
                                ...
                                /*conversation = findOrCreateConversation(account, bookmark.getJid(), true, null, 0);
                                conversation.setBookmark(bookmark);
                                joinMuc(conversation);*/
                            }
                        }
    ...
    public Conversation find(final Iterable<Conversation> haystack, final Account account,
                             final Jid jid, final int studentId) {
        if (jid == null) {
            return null;
        }
        for (final Conversation conversation : haystack) {
            if (studentId > 0) { //group messaging
                if ((account == null || conversation.getAccount() == account)
                        ...
                        && (conversation.getStudentId() == studentId)) {
                    return conversation;
                }
            } else {    //peer to peer
                if ((account == null || conversation.getAccount() == account)
    ...
    public Conversation findOrCreateConversation(final Account account, final Jid jid,
                                                 final boolean muc, final Jid groupJid,
                                                 final int isBroadcast) {
        return this.findOrCreateConversation(account, jid, muc, "", 0, 0, isBroadcast);
    }

    public Conversation findOrCreateConversation(final Account account, final Jid jid,
                                                 final boolean muc, final String groupName, final int groupId,
                                                 final int studId, final int isBroadcast) {
        return this.findOrCreateConversation(account, jid, muc, null, groupName, groupId, studId, isBroadcast);
    }
    ...
    public Conversation findOrCreateConversation(final Account account, final Jid jid, final boolean muc,
                                                 ...
                                                 final String groupName, final int groupId,
                                                 final int studId, final int isBroadcast
    ) {
        synchronized (this.conversations) {
            Conversation conversation = find(account, jid, studId);
            if (conversation != null) {
                return conversation;
            }
            conversation = databaseBackend.findConversation(account, jid, studId);
            if (conversation != null) {
                ...
                if (muc) {
                    /*conversation = new Conversation(groupName, account, jid,
                            Conversation.MODE_MULTI, groupId, studId);*/
                } else {
                    conversation = new Conversation(groupName, account, jid.toBareJid(),
                            Conversation.MODE_SINGLE, groupId, studId);
                }
            ...
            updateConversationUi();
            takeDbBackup(getApplicationContext());
            return conversation;
        }
    }
    ...
    public void updateAccount(final Account account) {  //account.initOtrEngine is already get executed onCreate method of this class.
    ...
    public void createAdhocConference(final Account account, final Iterable<Jid> jids, final UiCallback<Conversation> callback) {
        ...
        /*if (account.getStatus() == Account.State.ONLINE) {
            try {
                ...
                final Conversation conversation = findOrCreateConversation(account, jid, true, null, 0);
                joinMuc(conversation);
    ...
    public void markMessage(Message message, int status) {
        ...
        //SCH: Increment deliveryCount by 1 every time we get STATUS_SEND_RECEIVED
    ...
    //[Birender]: Save delivered message status into separate table "MessageStatus"
    public boolean markMessage(/*final Jid recipient, */final Account account, final String uuid,
                               Element packet, final int status) {
        if (uuid == null) {
            return false;
        } else {
            final Jid from = packet.getAttributeAsJid("from");

            if (packet.hasChild(CommonConstants.M_CUSTOM)) {
                Element customElement = packet.findChild(CommonConstants.M_CUSTOM);
                ...
                if (stuId != null) {
                    studId = Integer.parseInt(stuId);
                }

                for (Conversation conversation : getConversations()) {
                    if (conversation.getJid().toBareJid().toString().equals(groupJid)
                            && conversation.getAccount().equals(account)
                            && (conversation.getStudentId() == studId)) {
                        markMessage(conversation, uuid, status);
                        break;
                    }
                }
            }
            ...
            //Update MessageStatus table too
            MessageStatus messageStatus = new MessageStatus(uuid, from, status, System.currentTimeMillis());
            //add message status entry into MessageStatus table
            databaseBackend.updateMessageStatus(messageStatus);
            //NOTE: no need to call updateConversationUi() as its already called through markMessage() call above
            return true;
        }
    }
    ...
    public void sendReadMarker(final Conversation conversation) {
        ...
        if (confirmMessages() && markable != null && markable.getRemoteMsgId() != null) {
            ...
            Account account = conversation.getAccount();
            //TODO:Need to send read confirmation to the both parents-multiple counterparts
    ...
    public static void takeDbBackup(Context context) {
        try {
            Log.i("DB Backend", "Taking Database backup...");
            File sdCard = Environment.getExternalStorageDirectory();
            // File data = Environment.getDataDirectory();

            if (sdCard.canWrite()) {
                String currentDBPath = context.getDatabasePath("apphistory").getPath();
                Log.v("DB Backend", "Database path: " + currentDBPath);
                String backupDBPath = "apphistory.db";
                ...
                if (currentDB.exists()) {
                    ...
                    try {
                        ...
                        Log.i("DB Backend", "Backup saved to sd card with name apphistory.db");
                    } catch (Exception ex) {
                        Log.e("DB Backend", ex.getMessage());
                    } finally {
                        src.close();
                        dst.close();
                    }
                } else {
                    Log.i("DB Backend", "current Db path not exists/found.");
                }
            } else {
                Log.i("DB Backend", "cannot write/save to SD-Card");
            }
        } catch (Exception ex) {
            Log.e("DB Backend", ex.getMessage());
        }
